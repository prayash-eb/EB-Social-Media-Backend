...
export default class FollowService {
    public follow = async (followerId: mongoose.Types.ObjectId, followeeId: string) => {
        ...
        if (!follower) {
            throw new AppError("Follower does not exist", 404, "FOLLOW_MODULE");
        }
        if (!followee) {
            throw new AppError("Followee does not exist", 404, "FOLLOW_MODULE");
        }
    ...
    public getFollowings = async (userId: mongoose.Types.ObjectId) => {
        const userWithFollowings = await User.findById(userId)
