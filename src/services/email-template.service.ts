...
export default class EmailTemplateService {
    public addTemplate = async (templateInfo: CreateTempleteDTO) => {
        ...
        if (templateAlreadyExist) {
            throw new AppError("Template with same name already exists. You can update existing one", 400, "EMAIL_TEMPLATE_SERVICE")
        }
    ...
    public editTemplate = async (templateId: string, updatedData: EditTemplateDTO) => {
        const templateObjectId = new mongoose.Types.ObjectId(templateId);
        const template = await EmailTemplate.findByIdAndUpdate(templateObjectId, {
            ...updatedData
        }, { new: true })
        if (!template) {
            throw new AppError("Template doesnot exist or error while updating template", 404, "TEMPLATE_SERVICE")
        }
        return template
    }
