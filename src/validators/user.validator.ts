...
export const userAcademicEntrySchema = z.object({
    ...
    passedYear: z
        ...
        .max(currentYear, "Passed year cannot be in future"),
    degreeName: z
        .string()
        .trim()
        .toLowerCase()
        .min(2, "Degree name must be at least 2 characters")
        .max(50, "Degree name mustnot be greater than 50 characters"),
});

export const userAcademicsSchema = z.object({
    academicQualifications: z
        .array(userAcademicEntrySchema)
        .min(1, "Please provide at least one academic qualification."),
});
