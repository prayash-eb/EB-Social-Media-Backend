...
userSchema.methods.createTokens = function () {
    ...
    const ACCESS_EXPIRY_DATE = process.env.JWT_ACCESS_TOKEN_EXPIRY_DATE || "1d";
    const REFRESH_EXPIRY_DATE = process.env.REFRESH_TOKEN_EXPIRY_DATE || "7d";

    if (!JWT_ACCESS_SECRET || !JWT_REFRESH_SECRET) {
        throw new Error("JWT_SECRET is missing from the environment variables");
    }
    const jti = uuidV4();
    const accessToken = jwt.sign({ id: this._id, jti, email: this.email }, JWT_ACCESS_SECRET, {
        expiresIn: ACCESS_EXPIRY_DATE,
    } as SignOptions);
    const refreshToken = jwt.sign({ id: this._id, jti, email: this.email }, JWT_REFRESH_SECRET, {
        expiresIn: REFRESH_EXPIRY_DATE,
    } as SignOptions);
